...
namespace FoodDelivery.API.Controllers
{
    ...
    public class MenuItemsController : ControllerBase
    {
        ...
        public MenuItemsController(IMenuItemService service, IWebHostEnvironment env)
        {
            _service = service;
            _env = env;
        }
        ...
        [HttpGet]
        [AllowAnonymous]
        public async Task<ActionResult<PagedResult<MenuItemDto>>> GetAll(
            [FromQuery] QueryParams query, [FromQuery] int? restaurantId)
        {
            var result = await _service.GetAllAsync(query, restaurantId);
            return Ok(result);
        }
        ...
        public async Task<IActionResult> UploadImage(int id, IFormFile file)
        {
            ...
            var uploadsDir = Path.Combine(_env.WebRootPath ?? "wwwroot", "uploads", "menuitems");
            ...
            var imagePath = $"/uploads/menuitems/{fileName}";
